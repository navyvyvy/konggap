...
import { mapCrawledOffers, type CrawledOffer } from "./sources/insane-search";
...
export function payloadFromSnapshot(snapshot: OfferSnapshot, query: string, now = Date.now()) {
  ...
  const offers = sortOffersByFinalPrice(mapCrawledOffers(snapshot.offers, snapshot.fetchedAt).map(normalizeOffer));
